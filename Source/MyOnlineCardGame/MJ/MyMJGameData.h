...
// > 4 means no one throwed yet
#define FMyMJCoreDataPublicDirectDiceNumberNowMask_LastThrowAttender_BitPosiStart    (12)
#define FMyMJCoreDataPublicDirectDiceNumberNowMask_LastThrowAttender_BitLen          (4)

//because it is possible throw same values in multiple action, so this count distinguish whether new throw happens
#define FMyMJCoreDataPublicDirectDiceNumberNowMask_InGameDiceThrowCount_BitPosiStart    (16)
#define FMyMJCoreDataPublicDirectDiceNumberNowMask_InGameDiceThrowCount_BitLen          (8)
...
public:
    ...
    //use this fake key to let client do visual computation generating same result, it is likely a random value generated by server
    UPROPERTY(BlueprintReadOnly, meta = (DisplayName = "DiceNumberNowMask"))
    int32 m_iDiceVisualStateKey;
...
public:
    ...
    //when update reason part in the mask is valid, it means this delta want to update dice
    UPROPERTY(BlueprintReadOnly, meta = (DisplayName = "DiceNumberNowMask"))
    int32 m_iDiceNumberNowMask;
...
struct FMyMJDataAccessorCpp
{
    ...
    inline void setupDataExt(FMyMJDataStructCpp* pDataExt, bool bDebugIsFullMode = false)
    {
    ...
    inline const FMyMJDataStructCpp* getDataConst(bool verifyValid = true) const
    {
        const FMyMJDataStructCpp* ret = m_pDataExt;
        if (verifyValid) {
            MY_VERIFY(ret);
        }
        return ret;
    ...
    static void helperSetCoreDataDirtyRecordElemsDirty(FMyDirtyRecordWithKeyAnd4IdxsMapCpp &cDirtyRecord);
    static void helperSetCoreDataDirtyRecordDataDirty(FMyDirtyRecordWithKeyAnd4IdxsMapCpp &cDirtyRecord);
